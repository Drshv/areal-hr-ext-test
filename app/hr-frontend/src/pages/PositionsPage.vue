<template>
  <div class="q-pa-md">
    <div class="row q-mb-md">
      <div class="col-12">
        <h5 class="q-my-none">Должности</h5>
      </div>
    </div>

    <div class="row q-mb-md">
      <div class="col-12">
        <q-btn label="Добавить должность" color="primary" @click="createItem" />
      </div>
    </div>
    ...
    <q-dialog v-model="editDialog">
      <q-card style="min-width: 400px">
        <q-card-section>
          <div class="text-h6">{{ editForm.id ? 'Редактирование' : 'Новая должность' }}</div>
        </q-card-section>
...
const columns = [
  ...
  { name: 'organization', label: 'Организация', field: (row) => row.organization?.name || '-' },
...
const loadItems = async () => {
  loading.value = true;
  try {
    let url = `http://localhost:3000/api/positions?page=${pagination.value.page}&limit=${pagination.value.rowsPerPage}`;
...
const deleteItemConfirm = async () => {
  try {
    const response = await fetch(`http://localhost:3000/api/positions/${deleteItem.value.id}`, {
      method: 'DELETE',
      headers: getHeaders(),
    });
