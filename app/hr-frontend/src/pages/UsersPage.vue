<template>
  <div class="q-pa-md">
    <div class="row q-mb-md">
      <div class="col-12">
        <h5 class="q-my-none">Пользователи</h5>
      </div>
    </div>

    <div class="row q-mb-md">
      <div class="col-12">
        <q-btn label="Добавить пользователя" color="primary" @click="createItem" />
      </div>
    </div>
    ...
    <q-dialog v-model="editDialog">
      <q-card style="min-width: 400px">
        <q-card-section>
          <div class="text-h6">{{ editForm.id ? 'Редактирование' : 'Новый пользователь' }}</div>
        </q-card-section>
        ...
        <q-card-section>
          <q-form @submit="saveItem">
            ...
            <q-input v-model="editForm.login" label="Логин" lazy-rules :rules="[val => !!val || 'Обязательное поле']" />
            <q-input v-if="!editForm.id" v-model="editForm.password" label="Пароль" type="password" lazy-rules :rules="[val => !!val || 'Обязательное поле']" />
            <q-input v-else v-model="editForm.password" label="Новый пароль (оставьте пустым, если не менять)" type="password" />
    ...
    <q-dialog v-model="deleteDialog">
      <q-card>
        ...
        <q-card-section>
          Вы уверены, что хотите удалить {{ deleteItem?.login }}?
        </q-card-section>
...
const loadItems = async () => {
  loading.value = true;
  try {
    let url = `http://localhost:3000/api/users?page=${pagination.value.page}&limit=${pagination.value.rowsPerPage}`;
    if (filters.login) url += `&login=${encodeURIComponent(filters.login)}`;
...
const deleteItemConfirm = async () => {
  try {
    const response = await fetch(`http://localhost:3000/api/users/${deleteItem.value.id}`, {
      method: 'DELETE',
      headers: getHeaders(),
    });
